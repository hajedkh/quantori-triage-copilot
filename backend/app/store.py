...
class Run:
    ...
    candidates: list = field(default_factory=list)  # [{"smiles": str, "label": bool|None}]
    ...
    events: list = field(default_factory=list)  # append-only history of everything emit() has sent
    inbox: list = field(default_factory=list)  # operator guidance queued for the next loop iteration
    chat_history: list = field(default_factory=list)  # the copilot's own conversation for this run
